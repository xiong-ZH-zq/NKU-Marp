...
本模板改编自 Awesome-Marp 主题，在此感谢原作者的辛勤付出。

为使用本模板进行创作，请使用 vscode 或 Obsidian 进行创作，使用该模板前需要进行 Marp 的设置：

- vscode: 进入扩展设置并打开 `enable HTML` 选项；
- Obsidian: 使用  `Marp-Slides` 插件，并打开 `enable HTML` 选项；


相较原版更改的内容：

- 将主题色改为南开紫，十六进制码为 `#711a5f` ；
- 校徽改为NKU校徽；
- 示例PPT更注重数学与技术类内容。
...
<!-- _class: cover_e -->
<!-- _paginate: "" -->
<!-- _footer: ![](NKU_Banner.png) -->
<!-- _header: ![](NKU_Symbol.png) -->
...
<!-- _header: 目录<br>CONTENTS<br>![](NKU_Symbol.png)-->
<!-- _class: toc_b -->
<!-- _footer: "" -->
<!-- _paginate: "" -->
...
```python
import numpy as np  # 正常的 python 代码高亮
import pandas as pd    
